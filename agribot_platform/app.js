const CACHE_KEY = "agribot.latestRun.v5";
const SESSION_TOKEN_KEY = "agribot.platformToken";
...
function rememberPlatformToken(token) {
  try {
    ...
    // Private browsing or a locked-down browser may reject sessionStorage.
  }
}
...
function validateLatestPayload(payload) {
...
async function fetchLatest({ signal } = {}) {
  ...
  let response;
  try {
    response = await fetch("/api/runs/latest", { cache: "no-store", signal: request.signal });
  } finally {
    request.clear();
  }
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const payload = validateLatestPayload(await response.json());
  try {
    localStorage.setItem(CACHE_KEY, JSON.stringify({ savedAt: new Date().toISOString(), payload }));
  } catch {
    // Rendering live data must not fail just because browser storage is full.
  }
  return { payload, cached: false, fetchedAt: Date.now() };
}

async function saveFarmLayout(layout, { signal, allowPrompt = true } = {}) {
  ...
  let response;
  try {
    response = await fetch("/api/farm-layout", {
      method: "POST",
      headers,
  ...
  if (allowPrompt && (response.status === 401 || response.status === 403)) {
    const translatedPrompt = I18N?.t("authorizationRequired");
    const promptMessage = translatedPrompt && translatedPrompt !== "authorizationRequired"
      ? translatedPrompt
      : "Enter the dashboard access token:";
    const entered = window.prompt(promptMessage);
    if (entered && entered.trim() && entered.trim() !== token) {
      rememberPlatformToken(entered.trim());
      return saveFarmLayout(layout, { signal, allowPrompt: false });
    }
  }
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const result = await response.json();
  if (!result || result.ok !== true || !result.farm_layout) throw new Error("invalid layout response");
  return result;
}
...
function cachedLatest() {
  ...
  try {
    ...
    if (!Number.isFinite(savedAtMs) || Date.now() - savedAtMs > CACHE_MAX_AGE_MS) {
      localStorage.removeItem(CACHE_KEY);
      return null;
    }
    return { payload: validateLatestPayload(parsed.payload), cached: true, savedAt: parsed.savedAt, cacheAgeMs: Date.now() - savedAtMs };
  } catch {
    try {
      localStorage.removeItem(CACHE_KEY);
    } catch {
...
async function persistFarmLayout(layout, { refreshAfter = false } = {}) {
  ...
  const task = layoutSaveQueue.catch(() => undefined).then(async () => {
    // If a newer edit arrived while this request was waiting, never send the
    // obsolete snapshot after the newer one has already been queued.
    if (generation !== layoutSaveGeneration) return null;
  ...
  layoutSaveQueue = task.catch(() => undefined);
  return task;
}
...
function renderOverview(payload, data) {
  ...
  els.runStatus.textContent = data.cached ? I18N.t("cached") : summary.completed ? I18N.t("completed") : I18N.t("live");
  ...
  els.downloadLink.href = payload.csv_url || "#";
}
...
function renderFarmMap(decisions, data = {}) {
  els.farmRobotMap.replaceChildren();
  const latest = decisions[decisions.length - 1];
  const positionText = latest
    ? I18N.t("robotAt", { location: locationText(latest), label: I18N.label(latest.label) })
    : I18N.t("waitingLivePosition");
  els.liveLocation.textContent = data.cached ? `${I18N.t("cached")}: ${positionText}` : positionText;
...
async function refresh() {
  ...
  try {
    const data = await fetchLatest({ signal: refreshController.signal });
...
els.addRow.addEventListener("click", () => {
  try {
    ...
    render({ payload: { ...lastRenderedData?.payload, farm_layout: currentFarmLayout, decisions: lastRenderedData?.payload?.decisions || [] }, cached: lastRenderedData?.cached ?? true });
